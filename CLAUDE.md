...
A scalability-hardening pass landed after a full-codebase audit (three parallel investigations: DB/migrations, edge-function concurrency, frontend/realtime): `start-question`/`lock-question`/`show-leaderboard`/`end-session` now guard their status-transition `UPDATE` with `.eq('status', <the status just read>)` + `.select().maybeSingle()` instead of an unconditional update — closes a real race where a double-click or genuine concurrent request could pass validation twice and broadcast the same event (`question_end`/`question_start`/`leaderboard`/`game_over`) twice; see the gotcha below for the pattern to reuse on any future status-transition function. `create-session` and `submit-answer` now have rate limiting too (previously only `join-session`/`generate-quiz` did). `rate_limit_events` now gets opportunistically pruned (1-in-20 calls, 24h retention) since nothing was ever cleaning it. Routes are now code-split (`React.lazy` in `App.tsx`) so a landing-page visitor no longer downloads host/player game code. `quizzes(is_public)` and `rate_limit_events(created_at)` got supporting indices (migration `0008`).
...
- This Vercel project has repeatedly (twice observed) reported a successful deployment via the GitHub commit-status API ("Deployment has completed", `context: "Vercel"`) — and the Deployments dashboard even shows it with a filled blue "Production" badge — while the actual production domain kept serving the *previous* build (`X-Vercel-Cache: HIT` on a bundle hash that predates the new commit) for an indefinite period afterward. Both times, manually clicking "Redeploy" on that same deployment row in the dashboard fixed it immediately. Root cause not fully diagnosed (dashboard/API access only goes so far from this side) — treat "pushed and GitHub says Vercel deployed" as necessary but not sufficient; always verify the live bundle hash actually changed (`curl -s <url> | grep -oE '/assets/index-[A-Za-z0-9_-]+\.js'`) before declaring a fix live, and if it hasn't changed after a couple minutes, ask for a manual Redeploy rather than assuming it'll eventually catch up on its own.
- Edge functions with a status-transition shape (`start-question`, `lock-question`, `show-leaderboard`, `end-session`) originally read `session.status` once via `loadOwnedSession`, validated it in application code, then ran an unconditional `.update({...}).eq('id', sessionId)` — a read-then-write with no atomicity, so a double-click or genuine concurrent request could pass validation twice and both broadcast the same realtime event (players would see a duplicate reveal/leaderboard/podium). Found via a full-codebase scalability audit, not a bug report — reproduced live by firing two truly concurrent requests at a deployed `lock-question` and confirming two `question_end` broadcasts went out before the fix, one after. Fixed by adding `.eq('status', <the status just read>)` to the mutating update and switching to `.select('id').maybeSingle()`; if no row comes back, someone else already won the transition — return the existing `INVALID_STATUS` 409 instead of proceeding to broadcast. This is the same optimistic-concurrency shape `create-session`'s PIN-collision retry and `submit-answer`'s `answer_count` throttle already used correctly — any new status-transition edge function should use it from the start, not just an id-only `.eq()`.
